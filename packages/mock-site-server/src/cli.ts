  ...
  .action(async (dir: string, opts: Record<string, unknown>) => {
    try {
      ...
      const options: ServerOptions = {
        ...
        delay: opts.delay ? parseInt(opts.delay as string, 10) : undefined,
      ...
      console.log();
      console.log(
        pc.gray(`  Press ${pc.bold("Ctrl+C")} to stop`)
      );
      console.log();
    } catch (error) {
...
program
  ...
  .action(async (dir: string) => {
    try {
      ...
      console.log();
      console.log(
        pc.gray(`  Use ${pc.bold("mock-site-server serve <dir>/<site>")} to start a server`)
      );
      console.log();
    } catch (error) {
      console.error(pc.red(`\nError: ${(error as Error).message}`));

...
import { findMatchingVersions, fingerprintVendorBundles, type VendorBundleInput } from "./source-fingerprint.js";
 ...
 */
export async function isPublicNpmPackage(packageName: string): Promise<boolean> {
 ...
 */
export function extractNodeModulesPackages(files: Array<{ path: string }>): string[] {
...
export async function identifyInternalPackages(
  packageNames: string[],
  onProgress?: (checked: number, total: number, packageName: string, isInternal: boolean) => void
): Promise<Set<string>> {
...
export interface DependencyClassification {
  ...
  details: Map<string, {
    reason: 'workspace' | 'private-registry' | 'npm' | 'unknown-internal' | 'unknown-external';
    sourceLocation?: 'outside-node-modules' | 'in-node-modules' | 'no-source';
  }>;
}
 ...
 */
function detectWorkspacePackageRoots(sourceFiles: SourceFile[]): Map<string, string> {
  ...
  for (const file of sourceFiles) {
    ...
    const srcIndexMatch = file.path.match(/^(.+)\/([^/]+)\/src\/index\.[tj]sx?$/);
    if (srcIndexMatch) {
    ...
    const directIndexMatch = file.path.match(/^(.+)\/([^/]+)\/index\.[tj]sx?$/);
    if (directIndexMatch) {
  ...
  for (const file of sourceFiles) {
    ...
    if (srcMatch) {
      ...
      if (!packageRoots.has(packageName)) {
        ...
        potentialPackages.get(packageName)!.add(`${srcMatch[1]}/${packageName}`);
      }
    }
  }
  ...
  for (const file of sourceFiles) {
    ...
    const subdirIndexMatch = file.path.match(/^(.+)\/([^/]+)\/([^/]+)\/index\.[tj]sx?$/);
    if (subdirIndexMatch) {
...
export async function classifyDependencies(
  ...
  sourceFiles: SourceFile[],
  onProgress?: (checked: number, total: number, packageName: string, classification: string) => void
  ...
  for (const file of sourceFiles) {
    const nodeModulesMatch = file.path.match(/node_modules\/(@[^/]+\/[^/]+|[^/]+)/);
    if (nodeModulesMatch) {
  ...
  for (let i = 0; i < packagesNeedingNpmCheck.length; i += concurrency) {
    ...
    for (const { pkg, isPublic } of results) {
      ...
      const sourceLocation = inNodeModules ? 'in-node-modules' : 'no-source';
      
      if (isPublic) {
        result.external.add(pkg);
        result.details.set(pkg, {
          reason: 'npm',
          sourceLocation: sourceLocation as 'in-node-modules' | 'no-source',
        });
        onProgress?.(checked, totalToCheck, pkg, 'external');
      } else {
        result.internal.add(pkg);
        result.details.set(pkg, {
          reason: inNodeModules ? 'private-registry' : 'unknown-internal',
          sourceLocation: sourceLocation as 'in-node-modules' | 'no-source',
        });
 ...
 */
export function detectImportAliases(
  sourceFiles: SourceFile[]
): AliasMap {
...
export function generatePackageJson(
  ...
  if ((projectConfig?.environment === 'node' || projectConfig?.environment === 'both') && projectConfig?.hasTypeScript) {
  ...
  if (aliasMap && aliasMap.aliases.size > 0) {
    ...
    result._bundlerAliasConfig = {
      ...
      webpack: Object.fromEntries(
        Array.from(aliasMap.aliases.entries()).map(([alias, actual]) => [
          alias,
          actual
        ])
      ),
    ...
    notes.push(`${aliasMap.aliases.size} import aliases detected - see _importAliases for bundler configuration`);
  }
 ...
 */
export function detectProjectConfig(sourceFiles: SourceFile[]): DetectedProjectConfig {
  ...
  for (const file of sourceFiles) {
    ...
    if (!config.hasJsx && (/<[A-Z][a-zA-Z]*[\s/>]/.test(content) || /<\/[A-Z]/.test(content))) {
      config.hasJsx = true;
    }
    ...
    if (/from\s+['"]preact['"]|require\s*\(\s*['"]preact['"]/.test(content)) {
      preactImports++;
    }
    if (/from\s+['"]solid-js['"]|require\s*\(\s*['"]solid-js['"]/.test(content)) {
      solidImports++;
    }
    ...
    if (/\bimport\s+.*\s+from\s+['"]|export\s+(default\s+|{|\*|const|function|class|interface|type)/.test(content)) {
      hasEsm = true;
    }
    ...
    // Detect environment
    if (/\b(document|window|localStorage|sessionStorage|navigator|location|history|fetch|XMLHttpRequest|DOM|HTMLElement)\b/.test(content)) {
      hasBrowserApis = true;
    }
    if (/\b(process\.env|__dirname|__filename|require\.resolve|Buffer|fs\.|path\.)\b/.test(content)) {
      hasNodeApis = true;
    }
...
export function buildAliasPathMappings(
  ...
  for (const file of sourceFiles) {
    ...
    const pkgMatch = nodeModulesPath.match(/^node_modules\/(@[^/]+\/[^/]+|[^/]+)/);
 ...
 */
export function detectVendorBundleDirectories(sourceFiles: SourceFile[]): string[] {
  const topLevelDirs = new Map<string, { total: number; nodeModules: number }>();
  ...
  const packageNamePattern = /^(@[a-z0-9-]+\/)?[a-z0-9][-a-z0-9]*(-\d+\.\d+\.\d+.*)?$/i;
  ...
  for (const [dir, stats] of topLevelDirs) {
    ...
    const looksLikePackage = packageNamePattern.test(dir) && dir.includes('-');
...
export function generateTsConfig(
  ...
  const lib: string[] = [];
  if (projectConfig?.targetFeatures.optionalChaining || projectConfig?.targetFeatures.nullishCoalescing) {
    lib.push("ES2020");
  ...
  if (projectConfig?.environment === 'browser' || projectConfig?.environment === 'both') {
  ...
  // Determine module system
  const module = projectConfig?.moduleSystem === 'commonjs' ? "CommonJS" : "ESNext";
...
export async function generateDependencyManifest(
  ...
  options: {
    ...
    onVendorBundleProgress?: (completed: number, total: number, bundleFilename: string) => void;
    /** Progress callback for dependency classification */
    onClassificationProgress?: (checked: number, total: number, packageName: string, classification: string) => void;
  } = {}
): Promise<{
  packageJson: object;
  tsconfig: object;
  ...
  const projectConfig = extractedSourceFiles ? detectProjectConfig(extractedSourceFiles) : undefined;
  
  // Detect vendor bundle directories (third-party libraries that should be excluded from type checking)
  const vendorBundleDirs = extractedSourceFiles ? detectVendorBundleDirectories(extractedSourceFiles) : [];
  
  // Detect workspace packages for path mapping
  const workspacePackageRoots = extractedSourceFiles ? detectWorkspacePackageRoots(extractedSourceFiles) : new Map();
  const workspacePackages: WorkspacePackageMapping[] = Array.from(workspacePackageRoots.entries()).map(([name, path]) => ({
    name,
    relativePath: `./${path}`,
  }));
  ...
  if (extractionHash) {
    ...
    if (cachedManifest) {
      ...
      const cachedAliases = (cachedManifest.packageJson as Record<string, unknown>)._importAliases as Record<string, string> | undefined;
      const cachedAliasMap: AliasMap | undefined = cachedAliases 
        ? { aliases: new Map(Object.entries(cachedAliases)), evidence: new Map() }
        : undefined;
      ...
      return {
        ...
        tsconfig: generateTsConfig(aliasPathMappings, projectConfig, vendorBundleDirs, workspacePackages),
  ...
  if (extractedSourceFiles && extractedSourceFiles.length > 0) {
    ...
    for (const [packageName, result] of detectedVersions) {
      ...
      if (dep && !dep.version) {
        ...
        switch (result.source) {
          ...
          case 'custom-build': stats.bySource.customBuild++; break;
        }
    ...
    if (useFingerprinting) {
      ...
      if (vendorBundles.length > 0) {
        ...
        if (packagesStillNeedingVersions.length > 0) {
          onVersionProgress?.('vendor-bundle', `Fingerprinting ${vendorBundles.length} vendor bundles against ${packagesStillNeedingVersions.length} packages...`);

          // Convert vendor bundles to input format
          const vendorBundleInputs: VendorBundleInput[] = vendorBundles.map(vb => ({
            content: vb.content,
          ...
          const vendorResults = await fingerprintVendorBundles(
            ...
            packagesStillNeedingVersions,
            {
              ...
              includePrereleases,
              concurrency: 2,
              onProgress: (bundleFilename, packageName, result) => {
                vendorCompleted++;
                onVendorBundleProgress?.(vendorCompleted, vendorTotal, bundleFilename);
                if (result && packageName) {
                  onVersionProgress?.('vendor-bundle-matched', `${bundleFilename} -> ${packageName}@${result.version}`);
                }
              },
            }
          );
          ...
          for (const [_filename, result] of vendorResults) {
            const dep = analysis.dependencies.get(result.packageName);
            if (dep && !dep.version) {
              dep.version = result.version;
              dep.confidence = result.confidence;
      ...
      for (const [alias, actualPackage] of aliasMap.aliases) {
        if (analysis.dependencies.has(alias)) {
          ...
          if (!analysis.dependencies.has(actualPackage)) {
            ...
            let isActualPackageInternal = classification?.internal.has(actualPackage) ?? false;
            ...
            if (!classification?.internal.has(actualPackage) && !classification?.external.has(actualPackage)) {
              const isPublic = await isPublicNpmPackage(actualPackage);
            ...
            analysis.dependencies.set(actualPackage, {
              ...depInfo,
              name: actualPackage,
              isPrivate: isActualPackageInternal || depInfo.isPrivate,
            });
  ...
  const packageJson = generatePackageJson(outputName, analysis.dependencies, aliasMap, projectConfig);
  const tsconfig = generateTsConfig(aliasPathMappings, projectConfig, vendorBundleDirs, workspacePackages);

...
function satisfiesRange(version: string, range: string): boolean {
  ...
  if (range.includes(' ') && !range.includes('||')) {
    const parts = range.split(/\s+/).filter(p => p && !['', '-'].includes(p));
    ...
    if (hyphenIdx > 0) {
      ...
      return satisfiesRange(version, `>=${lower}`) && satisfiesRange(version, `<=${upper}`);
    }
  ...
  if (range.startsWith('^')) {
    ...
    if (rangeParts[0] === 0 && rangeParts[1] === 0) {
      return vParts[0] === 0 && vParts[1] === 0 && vParts[2] >= rangeParts[2];
    }
    // ^0.x - only minor updates
    if (rangeParts[0] === 0) {
      return vParts[0] === 0 && vParts[1] === rangeParts[1] && vParts[2] >= rangeParts[2];
    }
 ...
 */
function computeConfidence(range: string, matchCount: number): VersionConfidence {
  // Exact version match
  if (/^\d+\.\d+\.\d+$/.test(range)) {
    return 'exact';
  }
...
export async function inferFromKnownPeers(
  ...
  for (const unknownPkg of unknownPackages) {
    ...
    for (const [version, details] of Object.entries(metadata.versionDetails)) {
      if (!details.peerDependencies) {
        // No peer deps - can't infer, but doesn't exclude this version
        continue;
      }
      ...
      for (const [peerName, peerRange] of Object.entries(details.peerDependencies)) {
        peerTotalCount++;
    ...
    if (details?.peerDependencies) {
      for (const [peerName, range] of Object.entries(details.peerDependencies)) {
    ...
    const result: PeerDepResult = {
      ...
      inferredFrom,
      peerRange,
      matchingVersions: matchingVersions.slice(0, 5).map(m => m.version),
    };
...
export async function inferFromPeerRequirements(
  ...
  for (const unknownPkg of unknownPackages) {
    ...
    const versionScores = new Map<string, { maxSpec: number; peer: string; range: string }>();
    ...
    const sortedVersions = Array.from(versionScores.entries())
      .sort((a, b) => {
        if (b[1].maxSpec !== a[1].maxSpec) return b[1].maxSpec - a[1].maxSpec;
    ...
    const confidence = computeConfidence(bestInfo.range, sortedVersions.length);

...
/**
 * Generate environment variable definitions for Vite
 *
 * Vite's define option requires values to be valid JSON or JS expressions.
 * We use JSON.stringify() for all string values to ensure they're properly escaped.
 */
function generateDefineConfig(envVariables: EnvVariable[]): string {
    const defines: string[] = [
        "    'process.env.NODE_ENV': JSON.stringify('production')",
    ];

    for (const env of envVariables) {
        // Always wrap in JSON.stringify() to ensure valid define syntax
        const stringValue = env.defaultValue ?? '';
        const escapedValue = JSON.stringify(stringValue);
        defines.push(
            `    'process.env.${env.name}': JSON.stringify(${escapedValue})`,
        );
    }
...
export function generateViteConfig(options: ViteConfigOptions): string {
    ...
    const defineConfig = generateDefineConfig(envVariables);

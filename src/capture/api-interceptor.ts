...
const DEFAULT_OPTIONS: InterceptorOptions = {
  apiFilters: ["**/api/**", "**/graphql**", "**/v1/**", "**/v2/**", "**/v3/**"],
  captureBodies: true,
...
async function parseResponseBody(
  response: Response,
  maxSize: number
): Promise<{ body: unknown; bodyRaw?: string; bodyType: "json" | "text" | "binary" }> {
 ...
 * Parse request body
 */
function parseRequestBody(request: Request): { body?: unknown; bodyRaw?: string } {
...
export class ApiInterceptor {
  ...
  private async captureRequestResponse(
    ...
    // Extract URL pattern
    const { pattern, params } = extractUrlPattern(urlObj.pathname);

    // Build captured request
    const capturedRequest: CapturedRequest & { pattern: string; params: string[] } = {
      method,
      url,
    ...
    return {
      ...
      metadata: {
        ...
        responseTimeMs,
        sourcePageUrl: request.frame()?.url() || url,
      },
    };
  }

 ...
 */
function getParamName(segment: string, previousSegment: string | null, index: number): string {
...
export function extractUrlPattern(urlPath: string): UrlPatternResult {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (isDynamicSegment(segment)) {
      const paramName = getParamName(segment, previousSegment, paramIndex);
 ...
 */
export function generateFixtureId(method: string, pattern: string, index: number): string {

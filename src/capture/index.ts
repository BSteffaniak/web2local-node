...
import { BrowserManager, autoScrollPage, waitForNetworkIdle } from "./browser.js";
import { ApiInterceptor, deduplicateFixtures, sortFixturesByPriority } from "./api-interceptor.js";
import { StaticCapturer } from "./static-downloader.js";
import { generateServerManifest, generateCaptureSummary } from "../manifest/server-manifest.js";
import type { CaptureOptions, CaptureResult, ApiFixture, CapturedAsset } from "./types.js";

export { BrowserManager, autoScrollPage, waitForNetworkIdle } from "./browser.js";
export { ApiInterceptor, deduplicateFixtures, sortFixturesByPriority } from "./api-interceptor.js";
...
export { generateServerManifest, generateCaptureSummary } from "../manifest/server-manifest.js";
...
export async function captureWebsite(options: CaptureOptions): Promise<CaptureResult> {
  ...
  const apiInterceptor = new ApiInterceptor({
    ...
    onCapture: (fixture) => {
      opts.onProgress?.(`API: ${fixture.request.method} ${fixture.request.pattern}`);
    },
  });
  ...
  try {
    ...
    opts.onProgress?.(`Waiting ${opts.browseTimeout}ms for additional API calls...`);
  ...
  if (fixtures.length > 0 || assets.length > 0) {
    ...
    try {
      const manifestResult = await generateServerManifest(fixtures, assets, {
        name: hostname,
        sourceUrl: opts.url,

 ...
 */
export function resolveSourceMapUrl(cssUrl: string, sourceMapUrl: string): string {
...
export async function extractCssSourceMap(
  ...
  try {
    ...
    if (!content) {
      ...
      if (!response.ok) {
        result.errors.push(`Failed to fetch CSS from ${cssUrl}: ${response.status} ${response.statusText}`);
        return result;
      }
    ...
    if (isDataUri(sourceMapUrl)) {
      ...
      if (!extracted) {
        result.errors.push(`Failed to parse inline CSS source map from ${cssUrl}`);
        return result;
      }
      ...
      const response = await fetch(fullSourceMapUrl, { headers: BROWSER_HEADERS });
      if (!response.ok) {
        result.errors.push(`Failed to fetch CSS source map from ${fullSourceMapUrl}: ${response.status} ${response.statusText}`);
        return result;
      }
      ...
      try {
        ...
        result.errors.push(`Failed to parse CSS source map JSON from ${fullSourceMapUrl}: ${e}\n      Response preview: "${preview}${text.length > 1000 ? '...' : ''}"`);
        return result;
      }
    }
    ...
    if (!sourceMap.sources || !sourceMap.sourcesContent) {
      result.errors.push(`CSS source map from ${result.sourceMapUrl} is missing sources or sourcesContent arrays`);
      return result;
    }
...
/**
 * Scans source files for CSS module imports.
 *
...
export function findCssModuleImports(
  ...
  // Patterns for CSS module imports
  const patterns = [
    // ESM default import: import styles from './Button.module.scss'
    /import\s+(\w+)\s+from\s+['"]([^'"]+\.module\.(scss|sass|css|less))['"]/g,
    // ESM namespace import: import * as styles from './Button.module.scss'
    /import\s+\*\s+as\s+(\w+)\s+from\s+['"]([^'"]+\.module\.(scss|sass|css|less))['"]/g,
    // CommonJS require: const styles = require('./Button.module.scss')
    /(?:const|let|var)\s+(\w+)\s*=\s*require\s*\(\s*['"]([^'"]+\.module\.(scss|sass|css|less))['"]\s*\)/g,
  ];
  ...
  for (const file of sourceFiles) {
    for (const pattern of patterns) {
      ...
      while ((match = pattern.exec(file.content)) !== null) {
        ...
        // Extract used class names from this file
        const usedClassNames = extractUsedClassNames(file.content, variableName);
...
/**
 * Extracts class names used from a CSS module variable.
 *
 * Detects patterns like:
 *   styles.container
 *   styles['btn-text']
 *   styles["nav-item"]
 */
export function extractUsedClassNames(content: string, variableName: string): Set<string> {
  const classNames = new Set<string>();

  // Escape special regex characters in variable name
  const escapedVar = variableName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

  // Pattern 1: Dot notation - styles.container
  // Use word boundary to avoid matching stylesManager.add()
  const dotPattern = new RegExp(`\\b${escapedVar}\\.([a-zA-Z_][\\w-]*)\\b`, 'g');

  // Pattern 2: Bracket notation with quotes - styles['btn-text'] or styles["nav-item"]
  const bracketPattern = new RegExp(`\\b${escapedVar}\\[['"]([^'"]+)['"]\\]`, 'g');

  let match;

  // Extract dot notation usages
  while ((match = dotPattern.exec(content)) !== null) {
    // Filter out common method names that aren't CSS classes
    const className = match[1];
    if (!['toString', 'valueOf', 'hasOwnProperty'].includes(className)) {
      classNames.add(className);
    }
  }

  // Extract bracket notation usages
 ...
 */
export function generateCssModuleStub(importInfo: CssModuleImport): CssSourceFile {
  const resolvedPath = resolveImportPath(importInfo.sourceFile, importInfo.importPath);
 ...
 */
export function generateCssModuleDeclaration(importInfo: CssModuleImport): CssSourceFile {
  const resolvedPath = resolveImportPath(importInfo.sourceFile, importInfo.importPath) + '.d.ts';
...
export async function recoverCssSources(
  ...
  if (!skipTiers.has('source-map')) {
    ...
    for (const bundle of options.cssBundles) {
      const extractResult = await extractCssSourceMap(bundle.url, bundle.content);
  ...
  if (!skipTiers.has('stub')) {
    ...
    const existingCssFiles = new Set(result.sourceMapFiles.map((f) => f.path));
    ...
    result.stats.stubsGenerated = stubResult.stubs.filter((s) => s.source === 'stub').length;

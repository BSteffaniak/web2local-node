...
describe('Tier 1: CSS Source Map Extraction', () => {
    describe('findCssSourceMappingUrl', () => {
        ...
        test('should find inline data URI source map', () => {
            ...
            expect(findCssSourceMappingUrl(css)).toBe(
                'data:application/json;base64,abc123',
            );
        });
    ...
    describe('extractDataUriSourceMap', () => {
        ...
        test('should return null for invalid data URI', () => {
            expect(
                extractDataUriSourceMap('data:text/plain;base64,abc'),
            ).toBeNull();
    ...
    describe('extractCssSourceMap', () => {
        ...
        test('should extract sources from inline source map', async ({
            cssInlineSourceMap,
        }) => {
            ...
            expect(result.sourceMapUrl).toContain(
        ...
        test('should return empty files for CSS without source map', async () => {
            const result = await extractCssSourceMap(
                'https://example.com/styles/no-sourcemap.css',
            );
        ...
        test('should handle 404 for CSS file with URL in error message', async () => {
            server.use(
                create404Handler('https://example.com/missing/styles.css'),
            );

            const result = await extractCssSourceMap(
                'https://example.com/missing/styles.css',
            );
        ...
        test('should handle 404 for source map file with URL in error message', async () => {
            ...
            expect(result.sourceMapUrl).toBe(
                'https://example.com/styles/missing.css.map',
            );
            expect(result.files).toHaveLength(0);
            expect(result.errors.length).toBeGreaterThan(0);
            expect(result.errors[0]).toContain(
        ...
        test('should include URL and preview when source map returns HTML', async () => {
            server.use(
                ...
                http.get(
                    'https://example.com/styles/html-error.css.map',
                    () => {
...
describe('Tier 2: CSS Module Stub Generation', () => {
    describe('findCssModuleImports', () => {
        ...
        test('should find multiple import types', ({ multipleImports }) => {
            ...
            // Default import
            expect(
                imports.find((i) => i.importPath === './Page.module.scss'),
            ).toBeDefined();

            // Relative import
            expect(
                imports.find(
                    (i) => i.importPath === '../components/Button.module.css',
                ),
            ).toBeDefined();

            // Namespace import
            expect(
                imports.find((i) => i.importPath === './Header.module.sass'),
            ).toBeDefined();

            // Require
            expect(
                imports.find((i) => i.importPath === './Legacy.module.less'),
            ).toBeDefined();
        });
    ...
    describe('extractUsedClassNames', () => {
      ...
      `;
        ...
        test('should not match partial variable names', () => {
            // Use valid JS syntax (semicolon between statements)
            const content = 'stylesManager.add(); styles.real;';
            const classNames = extractUsedClassNames(content, 'styles');

            // Should only find 'real', not 'add' (from stylesManager)
    ...
    describe('generateCssModuleStub', () => {
        ...
        test('should add source file reference in comments', () => {
            ...
            expect(stub.content).toContain(
                'Used in: src/components/Button.tsx',
            );
        });
...
describe('recoverCssSources (Main Pipeline)', () => {
    ...
    test('should collect errors without throwing', async () => {
        server.use(
            createCssHandler(
                'https://example.com/styles/error.css',
                '.x{}',
                'error.css.map',
            ),
    ...
    test('should not generate stubs for files recovered from source maps', async () => {
        ...
        expect(
            result.sourceMapFiles.some((f) =>
                f.path.includes('Button.module.scss'),
            ),
        ).toBe(true);
        ...
        const stubPaths = result.stubFiles
            ...
            .map((f) => f.path);
        expect(stubPaths.some((p) => p.includes('Button.module.scss'))).toBe(
            false,
        );
    });
    ...
    test('should merge class names from multiple usages', async ({
        multipleImports,
    }) => {
